...
export const mostrarProductosPromo = (productos) => {
  
    productos.forEach(productos => {
        if (productos.seccion === 5) { 
            let idNombrePromo = "idCajasProdPromo"+productos.seccion;
            const contenedorProductoPromo = document.getElementById(idNombrePromo);
            const divPromo = document.createElement('div')
            divPromo.innerHTML += `<div class="col">
                                <div class="card h-100 shadow-sm">
                                    <img class="card-img-top" src="${productos.img}" alt="${productos.alt}">
                                    <div class="label-top shadow-sm">${productos.tituTipo}</div>
                                    <div class="card-body">
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-secondary"
                                                data-bs-dismiss="modal">Cancelar</button>
                                            <button type="button" class="btn btn-warning" id="botonCarPromo${productos.id}"
                                                data-bs-dismiss="modal">Confirmar</button>
                                        </div>
                                    </div>
                                </div>
            ...
            contenedorProductoPromo.appendChild(divPromo)            
        
            const botonPromo = document.getElementById(`botonCarPromo${productos.id}`)
...
export const mostrarSeccionPromo = (secProd) => {
    
    const contenedorCajaPromo = document.getElementById('producto-contenedorPromo');
    secProd.forEach(secProd => {
        if (secProd.id === 5) { 
            const sectionPromo = document.createElement('section')
            sectionPromo.innerHTML += `<section class="${secProd.nombre}">
                                    <div class="secTop">
                                        <h1 class="titulos" id="${secProd.titulo}">${secProd.titulo}</h1>
                                    <a href=#><i class="fa-solid fa-house fuentesNav"></i></a>
                                    </div>
                                    ...
                                    <div class="cajaProductos">
                                        <div class="container-fluid bg-trasparent my-4 p-3" style="position: relative;">
                                            <div class="row row-cols-1 row-cols-xs-2 row-cols-sm-2 row-cols-lg-4 g-3" id="idCajasProdPromo${secProd.id}">
                                            </div>
                                        </div>
                                    </div>
                            </section>`
            contenedorCajaPromo.appendChild(sectionPromo)
        }
    })
}
